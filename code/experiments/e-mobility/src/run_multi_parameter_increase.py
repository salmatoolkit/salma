...
def run_with_given_params(tlimit, vehicles, failure_prob):
...
if __name__ == '__main__':
    ...
    minTime, maxTime = map(int, sys.argv[3:5]) if len(sys.argv) >= 5 else (110, 110)
    minVehicles, maxVehicles = map(int, sys.argv[3:5]) if len(sys.argv) >= 5 else (110, 110)
    ...
    for p_failure in np.arange(minProb, maxProb + 0.05, 0.05):
        ...
        row = run_with_given_params(110, 5, p_failure)
        print(row)
        rows.append(row)
