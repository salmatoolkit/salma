...
test_1_ok :-
        ...
        (count(I,0,20) do
            ...
            printf("%    d =  %w - %w\n", [I, VerdictNow, Sched]),
            (I = 0 -> assertEqual(nondet, VerdictNow)
            	;
            	assertEqual(ok, VerdictNow)
            ),
            (
            	I = 0, assertEqual([], Sched), !
            	;
	            I < 11, assertEqual([r(f, 5, [s(0, 0) : nondet], nondet)], Sched), !
	            ;
	            I = 11, assertEqual([r(f, 5, [s(0, 0) : ok], ok)], Sched), !
	            ;
	            assertEqual([], Sched)
	         )
        ).

test_1_drop_one_at_5 :-
        init,
        F = forall([r,robot],
                   forall([j,item],
                          implies(
